...
* **pwd** (viết tắt của `print working directory`): Cho biế đường dẫn đến thư mục hiện tại đang truy cập.
...
* **rm** (remove): Cho phép xóa một file. Sử dụng `rm -r foo` directory nếu muốn xóa cả một thư mục.
...
Cách này tuy đơn giản nhưng phải thực hiện rất nhiều công đoạn. Cần xác thực các đối số, đảm bảo rằng kiểu của chúng là chính xác, bạn cần in phản hồi cho người dùng nếu họ không sử dụng chương trình một cách chính xác.
...
Biến parser sẽ lưu giữ các thông tin cần thiết để truyền các biến t đối dòng lệnh. Tham số `description` được sử dụng để cung cấp thông tin mô tả chương trình.
