...
* **pwd** (viết tắt của `print working directory`): Cho biết đường dẫn đến thư mục hiện tại đang truy cập.

* **cd** (viết tắt của `change directory`): Chuyển hướng tập tin.
  ```bash
  cd path/to/folder
  ```
  - `cd ..` (với 2 chấm) để chuyển lên 1 cấp thư mục trên.
  - `cd` để tới thẳng thư mục home.
  - `cd -` (với dấu gạch ngang) để chuyển tới thư mục đã ở trước đó.

* **ls**: Hiển thị danh sách các files hay thư mục con trong thư mục hiện tại đang truy cập.
...
* **cp** (`copy`): Cho phép copy một file.

  ```bash
  cp file.sh foo.sh
  ```
  - Ví dụ `cp file.sh foo.sh` sẽ tạo ra một bản copy của "file.sh" với tên là "foo.sh", nhưng "file.sh" sẽ vẫn còn ở đó.
  - Nếu muốn copy cả một thư mục, sử dụng lệnh `cp -r directory foo`.

* **mv** (`move`): Cho phép di chuyển một file sang một thư mục khác hoặc đổi tên của một file nào đó.
  ```bash
  mv foo.sh bar.sh
  ```
  - Ví dụ, `mv foo.sh bar.sh` sẽ đổi tên của file "foo.sh" thành "bar.sh".
  - `mv foo.sh ~/Pictures` sẽ di chuyển file "foo.sh" đến thư mục Pictures, nhưng không đổi tên của nó. Câu lệnh `mv foo.sh ~/Pictures/bar.sh` sẽ di chuyển file "foo.sh" đến thư mục Pictures với tên "bar.sh".

* **rm** (`remove`): Cho phép xóa một file. Sử dụng `rm -r someFolder` nếu muốn xóa cả một thư mục.

* **mkdir** (`make directory`): Cho phép tạo một thư mục mới.

* **history**: Hiển thị tất cả những lệnh đã gõ trước đó.

* **man**: Hiển thị hướng dẫn sử dụng của những lệnh khác. Ví dụ, muốn biết cách sử dụng lệnh `mkdir` như đã nói ở trên, thực hiện: `man mkdir`.
...
* **↑/↓**: Di chuyển ngược trở lại và chuyển tiếp thông qua các lệnh trước đó đã nhập vào phiên hiện tại.

  > Ngoài ra có thể nhấn phím F5 thay vì sử dụng phím mũi tên lên xuống để di chuyển ngược trở lại trên lịch sử lệnh.

* **F7**: Hiển thị lịch sử các lệnh trước đó. Ngoài ra có thể sử dụng phím mũi tên lên/xuống để lựa chọn bất kỳ một lệnh nào đó, sau đó nhấn Enter để thực hiện lệnh.
...
### 2.1 Đối số là gì?
Trong Python, có thể định nghĩa một hàm có số lượng đối số thay đổi.
...
Vì ta đã call hàm này và có truyền vào 2 tham số nên hàm đã chạy chính xác và không gặp lỗi.
...
Module `sys` trong Python cung cấp quyền truy cập vào bất kỳ đối số dòng lệnh nào thông qua `sys.argv`, danh sách các đối số dòng lệnh được chuyển tới chương trình Python.

Biến này đại diện cho tất cả các mục đi kèm qua đầu vào của input dòng lệnh, về cơ bản nó là một mảng giữ các đối số dòng lệnh của chương trình của chúng ta. Và nó đếm **bắt đầu bằng 0 chứ không phải 1**.
...
Cách truy cập đối số thông qua `sys.argv` tuy đơn giản nhưng phải thực hiện rất nhiều công đoạn. Cần xác thực các đối số, đảm bảo rằng kiểu của chúng là chính xác, bạn cần in phản hồi cho người dùng nếu họ không sử dụng chương trình một cách chính xác.

Python cung cấp một gói khác trong thư viện chuẩn để giải quyết: `argparse`.
...
Biến `parser` sẽ lưu giữ các thông tin cần thiết để truyền các biến đối dòng lệnh. Tham số `description` được sử dụng để cung cấp thông tin mô tả chương trình.

Ngoài `description` thì `ArgumentParser` còn một số các tham số khác như sau:

* `prog`: Tên của chương trình (mặc định `sys.argv[0]`, đây thường chính là tên file lưu code).
...
Ở vị trí đầu tiên (ở trong ví dụ là `integers` hoặc `--sum`), đây có thể là tên hoặc một flag sẽ truyền vào.
...
* **dest**: Tên của thuộc tính mà sẽ được thêm vào biến được trả về bởi `parse_args()`. `dest` ở trong ví dụ là accumulate, và có thể thấy thuộc tính này được gọi sau này.
* **const**: Một hằng giá trị, không được truyền vào từ đối dòng lệnh nhưng vẫn được lưu bên trong để một số hàm bên trong sử dụng, nhất là hàm `action`.
* **default**: Giá trị mặc định trả về nếu không có biến nào được truyền vào từ đối dòng lệnh.

Trong hàm `add_argument`, có 2 tham số đặc biệt cần lưu ý là `action` và `nargs`.

`action`: Mỗi tham số truyền vào đối dòng lệnh sẽ được lớp `ArgumentParser` đính với một `action`, tức là một hành động duy nhất.
...
`nargs`: Như đã nói ở trên thì mỗi tham số truyền vào đối dòng lệnh sẽ được lớp `ArgumentParser` đính với một `action`. Tham số `nargs` có thể đính một số lượng các tham số khác nhau với một `action`.
...
Hàm `parse_args()` biến các tham số được gửi vào từ đối dòng lệnh thành các thuộc tính của một object và trả về object đó. Như vậy có thể thấy trong ví dụ ở bên trên, `args` sẽ là một object chứa các biến được truyền vào như `integer` và `accumulate`.
